    ...
    return new Observable<UploadStatus>(subscriber => {
      entityService.uploadFiles(formData).subscribe({
        next: (httpEvent: HttpEvent<string[]>) => {
          switch (httpEvent.type) {
            ...
            case HttpEventType.Sent:
              ...
              subscriber.next(new UploadStatus('Uploading...', false, false));
              break;
            default:
        ...
        error: (error: HttpErrorResponse) => {
          ...
          subscriber.next(new UploadStatus('An error occurred while uploading your profile picture.', false, true));
        }
      });
    });
  }
